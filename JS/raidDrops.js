...
//       fullTable = getItems(tertiaryArray); // ← RUN getItems here too!
//     }
...
//     // if (dropTable === "tertiary") {
//     //   fullTable = getItems(raid.tertiaryDrops);
//     // } else {
//     //   fullTable = getItems(raid.dropTables[dropTable]);
//     // }
...
//Completely reworking generateRaidDrop. Keeping comment incase I mess it up real bad.

// function generateRaidDrop(raid, raidName, options = {}) {
//   let rolls = raid.doubleRoll ? 2 : raid.tripleRoll ? 3 : 1;
//   let drops = [];
//   let isExclusiveDrop = false;
...
//     if (Math.random() <= uniqueChance) {
//       const uniqueTableName = "unique";

//       if (raid.dropTables[uniqueTableName]) {
...
//       if (Math.random() < preRollChance) {
//         let preDropTable = raid.preRoll.table;
//         let preDrop = rollForRaidItem(raid, preDropTable);
...
//         dropTable = getRaidDropTable(raid);
//       } while (preRolled && dropTable === raid.preRoll.table);
//       let itemDrop = rollForRaidItem(raid, dropTable);
...
//   }
//   raidState.lastRaidRolled = raid;
//   raidState.lastRaidName = raidName;
...
//   // -------------------------------------
//   // Populate modal dynamically based on raid
//   // -------------------------------------
...
//   } else {
//     titleEl.textContent = "Configure Raid";
//     bodyEl.textContent = "No special configuration needed.";
//   }
...
//   // SEND RAID HANDLER depends on which UI we used
...
//       );
//       if (!selected) return alert("Please select difficulty.");
//       raidOptions.difficulty = selected.value; // "Normal" or "Hard"
//     }

//     // ToA will get its own options later

//     modal.style.display = "none";

//     // Store options so rollAgain() works
//     raidState.lastOptions = raidOptions;
//     sendRaid(raid, raidState.lastOptions);
//   });
...
// // Remove existing event listeners to prevent duplicate bindings
// document.querySelectorAll(".selectRaid").forEach((button) => {
//   button.replaceWith(button.cloneNode(true)); //Clone and replace to remove old listeners
// });
...
// function selectRaid(raid) {
//   // Customize behavior per-raid
//   if (!raid) return console.error("No raid supplied to selectRaid");

//   if (raid.name === "Chambers" || raid.key === "CoX") {
//     //show the points modal
//     showRaidPointsModal(raid);
//   } else if (raid.name === "Theatre" || raid.key === "ToB") {
//     //We'll put a ToB modal here
//   } else if (raid.name === "Tombs" || raid.key === "ToA") {
//     //We'll put a ToA modal here
//   } else {
//     //Setting a default, but want a modal for each raid

//     //What I really want is when a raid is selected to have each raid's specific modal pop up.
//     sendRaid(raid);
//   }
// }
...
function sendRaid(raid, options = {}) {
  ...
  // console.log("Raid object used:", raid);
  // console.log("Drop function:", raid.dropFunction);
  //Updating again

  // if (raid.dropFunction) {
  //   const drops = raid.dropFunction(raid, options);
  //   generateRaidDrop(drops, raid);
  // } else {
  //   console.error("No drop function defined for this raid:", raid.key);
  // }

  //Updating to above

  // console.log("Selected Raid (sendRaid):", raid, options);
  // // Storing last options here so rollAgain can function properly
  // raidState.lastOptions = options;
  // generateRaidDrop(raid, raid.name, options);
}

//commenting out sendRaid to create a more versatile function

// function sendRaid(raid) {
//   console.log("Selected Raid:", raid);
//   generateRaidDrop(raid, raid.name);
// }
